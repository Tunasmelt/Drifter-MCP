...
| F-11 | Replay store | ✅ Built, all three tiers | Exact + inverse (F-12) + semantic (F-13); indexes a whole CORPUS as of DEC-027(b), and `replay/coverage.py` projects how well that corpus generalizes. Still may not reach the fidelity floor against a real agent (§15 limitation 16) — now measured rather than assumed. Multi-session merging, cross-file last-writer-wins, fault/null-shape hits, and nested-key canonicalization now directly tested |
...
| F-37 | `drifter doctor` | ⚠️ Gate 1 scope + F-38's http check + F-26 classification + DEC-027(c) coverage | Surfaces unresolved classifications and below-floor projected replay coverage as `[WARN]`, neither counted against its exit code — not yet a hard live-mode gate (F-31/F-32 don't exist to gate against) |
