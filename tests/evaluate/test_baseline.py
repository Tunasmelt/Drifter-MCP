...
def test_provenance_breakdown_categorizes_every_call_into_exactly_one_bucket(tmp_path):
    ...
    assert result.provenance_breakdown == {"exact": 2, "inverse": 0, "semantic": 1, "synthetic": 1, "synthetic_miss": 0, "authored_fixture": 0, "unresolved": 1}
...
def test_provenance_breakdown_merges_counts_across_multiple_valid_runs(tmp_path):
    ...
    assert result.provenance_breakdown == {"exact": 1, "inverse": 0, "semantic": 1, "synthetic": 0, "synthetic_miss": 0, "authored_fixture": 0, "unresolved": 0}
...
def test_provenance_breakdown_excludes_calls_from_runs_that_were_themselves_excluded(tmp_path):
    ...
    assert len(result.excluded_runs) == 1
    assert result.provenance_breakdown == {"exact": 1, "inverse": 0, "semantic": 0, "synthetic": 0, "synthetic_miss": 0, "authored_fixture": 0, "unresolved": 0}
