...
from mcp_drifter.cli.config import ConfigError, ServerConfig
...
def _render_drifter_yaml(servers: list[ServerConfig]) -> str:
    header = (
        ...
        "#\n"
        "# `drifter observe` reads `servers:` and `record:`. `drifter run` also\n"
        "# needs an `agent:` block, and `tasks:` for task assertions.\n"
        "# `drifter init` itself does not classify tool risk; `drifter doctor`\n"
        "# reports classification (F-26). See docs/SPEC.md §15 for stated\n"
        "# limitations. Review the server list below before running\n"
        "# `drifter observe` against it.\n\n"
    )
    body = {
        "version": 1,
